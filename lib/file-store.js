...
function FileCookieStore(filePath, encryptionKey) {
    ...
    loadFromFile(this.filePath, this.cryptr, function(dataJson) {
        if (dataJson)
            self.idx = dataJson;
    })
}
...
FileCookieStore.prototype.putCookie = function(cookie, cb) {
    ...
    saveToFile(this.filePath, this.cryptr, this.idx, function() {
        cb(null);
    });
};
...
var _writing_ = {};
function writeFile(filePath, cryptr, data, done) {
    var dataJson = cryptr === null ? JSON.stringify(data) : cryptr.encrypt(JSON.stringify(data));
    ...
    fs.writeFile(filePath, dataJson, function (err) {
        ...
        if ( wo.next.length ) {
            writeFile(filePath, cryptr, wo.data, wo.next);
        }
        else {
...
function saveToFile(filePath, cryptr, data, cb) {
    var wo = _writing_[filePath];
    // If not writing to this file, start writing right now
    if ( !wo ) {
        writeFile(filePath, cryptr, data, [cb]);
    }
...
function loadFromFile(filePath, cryptr, cb) {
    ...
    if (data) {
        try {
            if (cryptr) {
                if (data.startsWith('{') && data.endsWith('}')){
                    throw new Error('Cookie file ' + filePath + ' is not encrypted. Please change the file or remove the encryption key.');
                }
            } else {
                if (!data.startsWith('{') || !data.endsWith('}')){
                    throw new Error('Cookie file ' + filePath + ' is encrypted. Please change file or add an encryption key.');
                }
            }
        } catch (e) {
            throw e;
        ...
        try {
            var dataJson = cryptr === null ? JSON.parse(data) : JSON.parse(cryptr.decrypt(data));
        } catch (e) {
